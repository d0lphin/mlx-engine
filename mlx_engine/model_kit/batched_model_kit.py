...
class BatchedModelKit:
    ...
    def _fetch_cache_with_checkpoint_fallback(
        ...
        if result is not None:
            ...
            # No checkpoint match either — full prefill from scratch
            fresh_cache = make_prompt_cache(self.model, self._max_kv_size)
            # Prefill all but the last token
            tokens_to_prefill = tokens_array[:-1] if tokens_array.size > 1 else mx.array([], dtype=tokens_array.dtype)
            ...
            if tokens_to_prefill.size > 0:
                self._prefill_with_checkpoints(
                    ...
                    full_prompt_tokens=tokens_array,
                    rqueue=rqueue,
                    cached_tokens_before=0,
                )

            # Save checkpoint of the prefilled cache (key = tokens that are in the cache)
            if tokens_to_prefill.size > 0:
                self._checkpoint_store.save(tokens_array[:tokens_to_prefill.size], fresh_cache)

            return fresh_cache, prompt_tokens[-1:]
